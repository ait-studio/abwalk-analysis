...
def main():
    ...
    while True:
        ...
        modeChoice = int(
            input("1. Standard mode\t2. Light mode(use tiny weights and cfg)\n")
        )

        if modeChoice == 1:
            net = cv2.dnn.readNet(
                f"{weightDirectory}/yolov3.weights", f"{cfgDirectory}/yolov3.cfg"
            )
            break
        elif modeChoice == 2:
            net = cv2.dnn.readNet(
                f"{weightDirectory}/yolov3-tiny.weights",
                f"{cfgDirectory}/yolov3-tiny.cfg",
            )
            break
        elif modeChoice == -1:
    ...
    if modeChoice == 1:
        ...
        while True:
            print("Program quit with enter -1")
            sizeChoice = int(
                input("1. smallest(fast)\t2.middle(normal)\t3.biggest(slow)\n")
            )
    ...
    for i in listDir:
        ...
        outFile = cv2.VideoWriter(
            outFileName, fourcc, frameRate, (frameWidth, frameHeight)
        )
        ...
        while True:
            ...
            if ret:
                ...
                blob = cv2.dnn.blobFromImage(
                    img, 0.00392, modelSize, (0, 0, 0), True, crop=False
                )
                net.setInput(blob)
                ...
                percentageDigit = (
                    1 if realPercentage < 10 else (2 if realPercentage < 100 else 3)
                )
                ...
                print(
                    "Progress |",
                    progress,
                    f"| {curFrame:04} / {int(frameCount):04}",
                    end="\r",
                )
            else:
                break
